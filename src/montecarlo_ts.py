...
class MonteCarloTS(ai_interface.AiAlgorithmIf):
    ...
    def _tree_policy(self, node):
        ...
        # TODO could this get stuck finding the same leaf node?
        # what if a move is no longer available e.g. hole closing or becomming a child
        # does progressing down the game tree prevent this (don't think so)
    ...
    def _expand(self, pnode):
        ...
        if new_state in self.node_dict:
            ...
            reward = 1.0 if self.game.get_turn() == self.my_turn_id else 0.0
