...
class Owners(bhv.BehaviorGlobal):
    ...
    def fill_it(self, game_ui):
        ...
        tk.Label(status, text=gi.PLAYER_NAMES[True]).pack(side=tk.LEFT)
        self._top_dev = tk.Label(status, text='   0')
        self._top_dev.pack(side=tk.LEFT)
        tk.Label(status, text=gi.PLAYER_NAMES[False]).pack(side=tk.LEFT)
...
class SelectOwnedHoles(bhv.BehaviorIf):
    ...
    def ask_mode_change(cls, game_ui):
        ...
        cls.facing = game_ui.tkvars.facing_players.get()
        game_ui.tkvars.facing_players.set(False)
        game_ui.show_toggle_facing()
    ...
    def leave_mode(cls, game_ui):
        ...
        game_ui.tkvars.facing_players.set(cls.facing)
        game_ui.show_toggle_facing()
        return True
